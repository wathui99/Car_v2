...
void speed(float left, float right, float persent) { //max left la 255, max right la 255
	left*=((persent/100)*ratio*special_ratio); //100
	right*=((persent/100)*ratio*special_ratio);
	if (left>=0) {
		sbi(PORTD,DIR10);
		cbi(PORTD,DIR11);
	...
	while(1)
	{
		...
		if(get_button(BTN0)) {
			while(1) {
				...
				print();
				if(get_button(BTN1)) {
					if ((ratio_base+0.05) > 1.0) ratio_base=0.4;
					else ratio_base+=0.05;
					ratio=ratio_base;
				}
				if(get_button(BTN0)) {
					if ((ratio_base-0.05) < 0.4) ratio_base=1.0;
					else ratio_base-=0.05;
					ratio=ratio_base;
				}
