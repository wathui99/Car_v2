...
#define DIR10	3
#define DIR11   6
#define SERVO_CENTER		2800	//Sai số của cần sensor trên xe (am se dich qua trai, duong se dich qua phai)
#define ANGLE_MAX           120
#define STEP				5

volatile float ratio;
volatile float special_ratio;
volatile float ratio_base;
volatile float PID_ratio;
volatile uint8_t pulse_v_des;
...
void speed(float left, float right, float persent) { //max left la 255, max right la 255
	left*=((persent/100)*ratio*special_ratio*PID_ratio); //100
	right*=((persent/100)*ratio*special_ratio*PID_ratio);
	if (left>=0) {
		sbi(PORTD,DIR10);
		cbi(PORTD,DIR11);
...
void sel_mode()
{
	...
	while(1)
	{
		...
		if(get_button(BTN0)) {
			led7(2222);
			_delay_ms(1000);
			while(1) { //lay ti so van toc goc
				led7((int)(ratio_base*100));
				print();
				if(get_button(BTN1)) {
					if ((ratio_base+0.05) > 1.0) ratio_base=0.1;
					else ratio_base+=0.05;
					ratio=ratio_base;
				}
				if(get_button(BTN0)) {
					if ((ratio_base-0.05) < 0.1) ratio_base=1.0;
					else ratio_base-=0.05;
					ratio=ratio_base;
				}
			...
			while (1) // lay toc do pulse encoder mong muon
			{
				...
				print();
				if(get_button(BTN1)) {
					if ((pulse_v_des+1) > 100) pulse_v_des=1;
					else pulse_v_des++;
				}
				if(get_button(BTN0)) {
					if ((pulse_v_des-1) <= 0) pulse_v_des=100;
					else pulse_v_des--;
				}
				if(get_button(BTN2)) break;
			}
